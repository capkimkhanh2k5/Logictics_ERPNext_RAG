(()=>{console.log("SMART WORKFLOW WIDGET SCRIPT LOADED");$(document).ready(function(){let k=[{doctype:"Material Request",id:"wiz-Material-Request",slug:"material-request",label:"1. Y\xEAu c\u1EA7u mua h\xE0ng (Material Request)"},{doctype:"Purchase Order",id:"wiz-Purchase-Order",slug:"purchase-order",label:"2. \u0110\u01A1n \u0111\u1EB7t h\xE0ng (Purchase Order)"},{doctype:"Shipment Tracking",id:"wiz-Shipment-Tracking",slug:"shipment-tracking",label:"3. Theo d\xF5i h\xE0nh tr\xECnh (Shipment Tracking)"},{doctype:"Purchase Receipt",id:"wiz-Purchase-Receipt",slug:"purchase-receipt",label:"4. Nh\u1EADn h\xE0ng (Purchase Receipt)"},{doctype:"Landed Cost Voucher",id:"wiz-Landed-Cost-Voucher",slug:"landed-cost-voucher",label:"5. Ph\xE2n b\u1ED5 gi\xE1 v\u1ED1n (Landed Cost)"},{doctype:"Stock Entry",id:"wiz-Stock-Entry",slug:"stock-entry",label:"6. Nh\u1EADp kho (Stock Entry)"}],T=k.map(e=>e.doctype),a=null,_=null,x=[],m=null,g=null;function R(e){return e==="Air"?`
        ...
        `),frappe.call({method:"logistics_wizard.api.get_active_shipments",callback:function(t){if(t.message&&t.message.status==="success"){let l=t.message.data||[];if(l.length===0){e.html(`
                    ...
                    `;l.forEach(o=>{n+=`
                        ...
                        `}),n+="</div>",e.html(n),$(".lw-shipment-item").hover(function(){$(this).css({background:"#eef5ff","border-color":"#b8d5fd"})},function(){$(this).css({background:"#f8f9fa","border-color":"#e9ecef"})}).on("click",function(){let o=$(this).data("name");F(o,"Purchase Order")})}else e.html('<div style="text-align: center; color: red; padding: 20px;">L\u1ED7i t\u1EA3i danh s\xE1ch v\u1EADn chuy\u1EC3n.</div>')}})}function F(e,t){$("#lw-shipment-content").html(`
        ...
        `),$("#lw-btn-back-shipments").on("click",function(){if(g&&(cancelAnimationFrame(g),g=null),a){try{a.remove()}catch(n){}a=null,m=null}S()}),frappe.call({method:"logistics_wizard.api.get_shipment_tracking",args:{docname:e,doctype:t},callback:function(n){n.message&&n.message.status==="success"?q(n.message.data,e,t):$("#lw-map-info-text").html(`<span style="color: #d9534f;">${n.message?n.message.message:"Kh\xF4ng th\u1EC3 t\u1EA3i th\xF4ng tin l\u1ED9 tr\xECnh."}</span>`)}})}function O(e){let t=window.L||window.leaflet;if(!t||!a)return;e==="Ocean"||e==="Sea"?(m||(m=t.tileLayer("https://tiles.openseamap.org/seamark/{z}/{x}/{y}.png",{attribution:'Map data: &copy; <a href="http://www.openseamap.org">OpenSeaMap</a> contributors',maxZoom:18,opacity:1})),a.hasLayer(m)||m.addTo(a)):m&&a.hasLayer(m)&&a.removeLayer(m)}function C(e,t,l){let[n,o]=e,[i,s]=t,[d,c]=l,r=c-s,p=d-i;r>180&&(r-=360),r<-180&&(r+=360);let u=o-s,w=n-i;u>180&&(u-=360),u<-180&&(u+=360);let y=r*r+p*p;if(y===0)return Math.hypot(u,w);let f=Math.max(0,Math.min(1,(u*r+w*p)/y)),b=f*r,v=f*p;return Math.hypot(u-b,w-v)}function M(e,t=.002){if(!e||!Array.isArray(e)||e.length<=2)return e||[];let l=0,n=0,o=e.length-1;for(let i=1;i<o;i++){let s=C(e[i],e[0],e[o]);s>l&&(l=s,n=i)}if(l>t){let i=M(e.slice(0,n+1),t),s=M(e.slice(n),t);return i.slice(0,i.length-1).concat(s)}else return[e[0],e[o]]}function I(e,t,l,n){let o=Math.PI/180,i=180/Math.PI,s=e*o,d=l*o,c=(n-t)*o;for(;c>Math.PI;)c-=2*Math.PI;for(;c<-Math.PI;)c+=2*Math.PI;let r=Math.sin(c)*Math.cos(d),p=Math.cos(s)*Math.sin(d)-Math.sin(s)*Math.cos(d)*Math.cos(c);return(Math.atan2(r,p)*i+360)%360}function z(e){let t=[0],l=0;if(!e||e.length<=1)return{cumulative:[0],total:0};for(let n=0;n<e.length-1;n++){let[o,i]=e[n],[s,d]=e[n+1],c=d-i;c>180&&(c-=360),c<-180&&(c+=360);let r=s-o,p=Math.cos((o+s)/2*Math.PI/180);l+=Math.hypot(r,c*p),t.push(l)}return{cumulative:t,total:l}}function P(e,t,l){if(!e||e.length===0)return{point:[0,0],bearing:0};if(e.length===1)return{point:e[0],bearing:0};let n=Math.max(0,Math.min(1,l));if(t.total===0)return{point:e[0],bearing:0};let o=n*t.total,i=0;for(let h=0;h<t.cumulative.length-1;h++)if(o>=t.cumulative[h]&&o<=t.cumulative[h+1]){i=h;break}i>=e.length-1&&(i=e.length-2);let s=t.cumulative[i],d=t.cumulative[i+1]-s,c=d===0?0:Math.max(0,Math.min(1,(o-s)/d)),r=e[i],p=e[i+1]||r,u=r[0]+c*(p[0]-r[0]),w=r[1],f=p[1]-w;f>180&&(f-=360),f<-180&&(f+=360);let b=w+c*f,v=I(r[0],r[1],p[0],p[1]);return{point:[u,b],bearing:v}}function D(e,t,l,n=1500){if(g&&(cancelAnimationFrame(g),g=null),!e||!t||t.length===0)return;let o=z(t),i=typeof e._currentProgress=="number"?e._currentProgress:0,s=Math.max(0,Math.min(1,l)),d=performance.now();function c(r){let p=r-d,u=n<=0?1:Math.min(1,p/n),w=1-Math.pow(1-u,3),y=i+(s-i)*w,{point:f,bearing:b}=P(t,o,y);e.setLatLng(f);let v=e.getElement?e.getElement():null;if(v){let h=v.querySelector(".lw-vehicle-icon-svg")||v.querySelector("svg");h&&(h.style.transform=`rotate(${b}deg)`,h.style.transformOrigin="center center")}e._currentProgress=y,u<1?g=requestAnimationFrame(c):g=null}g=requestAnimationFrame(c)}function q(e,t,l){let n=window.L||window.leaflet;if(!n){$("#lw-map-info-text").html('<span style="color: red;">Th\u01B0 vi\u1EC7n b\u1EA3n \u0111\u1ED3 (Leaflet) ch\u01B0a \u0111\u01B0\u1EE3c t\u1EA3i.</span>');return}if(g&&(cancelAnimationFrame(g),g=null),a){try{a.remove()}catch(G){}a=null,m=null}x=[],_=null,a=n.map("shipment-map",{zoomControl:!0,attributionControl:!0}).setView([20,150],3),n.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',subdomains:["a","b","c"],maxZoom:19}).addTo(a),O(e.method);let o=e.full_route&&e.full_route.length>0?e.full_route:e.route||[];if(!o||o.length===0){$("#lw-map-info-text").html('<span class="text-muted">Ch\u01B0a c\xF3 t\u1ECDa \u0111\u1ED9 n\xE0o \u0111\u01B0\u1EE3c ghi nh\u1EADn cho \u0111\u01A1n h\xE0ng n\xE0y.</span>');return}let i=M(o,.002);(!i||i.length<2)&&(i=o);let s={color:e.method==="Air"?"#007AFF":e.method==="Ocean"?"#0055B3":"#28a745",weight:3.5,opacity:.85,dashArray:e.method==="Air"?"6, 8":e.method==="Ocean"?"8, 4":""};_=n.polyline(i,s).addTo(a);try{a.fitBounds(_.getBounds(),{padding:[40,40],maxZoom:8})}catch(G){}let d=i[0],c=n.divIcon({className:"custom-origin-icon",html:'<div style="background: #28a745; color: white; border-radius: 50%; width: 26px; height: 26px; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: bold; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3);">A</div>',iconSize:[26,26],iconAnchor:[13,13]}),r=n.marker(d,{icon:c}).addTo(a);r.bindPopup(`<b>\u0110i\u1EC3m xu\u1EA5t ph\xE1t (Origin):</b><br>${d[0].toFixed(2)}, ${d[1].toFixed(2)}<br><small>${e.origin||""}</small>`),x.push(r);let p=i[i.length-1],u=n.divIcon({className:"custom-dest-icon",html:'<div style="background: #dc3545; color: white; border-radius: 50%; width: 26px; height: 26px; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: bold; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3);">B</div>',iconSize:[26,26],iconAnchor:[13,13]}),w=n.marker(p,{icon:u}).addTo(a);w.bindPopup(`<b>\u0110i\u1EC3m \u0111\u1EBFn (Destination):</b><br>${p[0].toFixed(2)}, ${p[1].toFixed(2)}<br><small>${e.destination||""}</small>`),x.push(w);let y=typeof e.progress=="number"?e.progress:.5,f=z(i),b=P(i,f,0),v=n.divIcon({className:"custom-vehicle-icon",html:`
            ...
            `,iconSize:[40,40],iconAnchor:[20,20]}),h=n.marker(b.point,{icon:v,zIndexOffset:1e3}).addTo(a);h._currentProgress=0,h.bindPopup(`<b>V\u1ECB tr\xED hi\u1EC7n t\u1EA1i:</b><br>${e.current_location||"\u0110ang v\u1EADn chuy\u1EC3n"}`),x.push(h),D(h,i,y,1500),setTimeout(()=>{h&&a&&a.hasLayer(h)&&h.openPopup()},1500);let N=e.method==="Air"?"\u2708\uFE0F \u0110\u01B0\u1EDDng h\xE0ng kh\xF4ng (Air)":e.method==="Ocean"?"\u{1F6A2} \u0110\u01B0\u1EDDng bi\u1EC3n (Ocean)":"\u{1F69A} \u0110\u01B0\u1EDDng b\u1ED9 (Road)",W=e.distance_km?` &bull; ${Math.round(e.distance_km).toLocaleString()} km`:"",K=`
        ...
        `;$("#lw-map-info-text").html(K),H(t,l),setTimeout(()=>{a&&a.invalidateSize()},300)}function H(e,t){let l=$("#lw-timeline-container");if(window.cur_frm&&cur_frm.doc&&cur_frm.doc.name===e&&cur_frm.doc.transit_route&&cur_frm.doc.transit_route.length>0){L(cur_frm.doc.transit_route,l);return}let n=t,o=e;t==="Purchase Order"?frappe.db.get_value("Shipment Tracking",{purchase_order:e},"name").then(i=>{i&&i.message&&i.message.name?frappe.db.get_doc("Shipment Tracking",i.message.name).then(s=>{L(s.transit_route||[],l)}):l.html('<div style="color: #8d99a6; font-size: 12px; padding: 8px 0;">Ch\u01B0a li\xEAn k\u1EBFt phi\u1EBFu Shipment Tracking ho\u1EB7c ch\u01B0a c\xF3 l\u1ED9 tr\xECnh chi ti\u1EBFt.</div>')}):t==="Shipment Tracking"?frappe.db.get_doc("Shipment Tracking",e).then(i=>{L(i.transit_route||[],l)}):l.html('<div style="color: #8d99a6; font-size: 12px; padding: 8px 0;">Kh\xF4ng c\xF3 d\u1EEF li\u1EC7u l\u1ED9 tr\xECnh.</div>')}function L(e,t){if(!e||e.length===0){t.html('<div style="color: #8d99a6; font-size: 12px; padding: 8px 0;">Ch\u01B0a c\xF3 tr\u1EA1m l\u1ED9 tr\xECnh n\xE0o.</div>');return}let l='<div class="lw-timeline" style="margin-top: 5px;">';e.forEach((n,o)=>{let i=o===e.length-1;l+=`
            ...
            `}),l+="</div>",t.html(l)}function A(){k.forEach(e=>{let t=$("#"+e.id);if(t.length){t.removeClass("wiz-step-completed wiz-step-current wiz-step-pending"),t.find(".wiz-check-badge").html("");let l=t.find("a");l.attr("href","/app/"+e.slug),l.text(e.label)}})}function V(e){!e||!e.length||e.forEach(t=>{let l=k.find(d=>d.doctype===t.doctype);if(!l)return;let n=$("#"+l.id);if(!n.length)return;let o=t.label||l.label,i=t.url||"/app/"+l.slug,s=n.find("a");s.attr("href",i),s.text(o),t.completed?(n.addClass("wiz-step-completed"),n.find(".wiz-check-badge").html("\u2714")):t.is_current?n.addClass("wiz-step-current"):n.addClass("wiz-step-pending"),t.is_current&&n.addClass("wiz-step-current")})}function E(){if(B(),typeof frappe=="undefined"||!frappe.get_route)return;let e=frappe.get_route();if(!(!e||!e.length))if(e[0]==="Form"&&e[1]&&T.includes(e[1])&&e[2]){let t=e[1],l=e[2];frappe.call({method:"logistics_wizard.api.get_workflow_chain_status",args:{doctype:t,docname:l},callback:function(n){A(),n&&n.message&&n.message.success&&V(n.message.steps)}})}else if(e[0]==="List"&&e[1]&&T.includes(e[1])){A();let t="wiz-"+e[1].replace(/\s+/g,"-");$("#"+t).addClass("wiz-step-current")}else A()}typeof frappe!="undefined"&&frappe.router&&frappe.router.on("change",E),window.LogisticsWizardMap={douglasPeucker:M,perpendicularDistance:C,calculateBearing:I,computePolylineMetrics:z,interpolateAtProgress:P,animateVehicle:D,update_marine_overlay:O,getShipmentMap:function(){return a},getSeaOverlayLayer:function(){return m},getMapPolyline:function(){return _},getMapMarkers:function(){return x}},setTimeout(E,300)});})();
//# sourceMappingURL=smart_workflow_widget.bundle.UYWM4X7Z.js.map
